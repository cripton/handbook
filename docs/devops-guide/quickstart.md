...
**Proxy-Server (nginx):**
If the installation is behind a proxying nginx server, remove `/etc/nginx/modules-enabled/60-jitsi-meet.conf`. Then go to `/etc/nginx/sites-available/your-conf` and change it to listen on 443 instead of 4444 and restart nginx.
